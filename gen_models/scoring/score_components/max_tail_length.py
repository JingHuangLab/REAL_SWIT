...
class MaxTailLength(BaseScoreComponent):
    ...
    def calculate_score(self, molecules: List) -> ComponentSummary:
        ...
        for mol in molecules:
            if not mol:
                ...
                score = self.max_tail_length(mol)
            scores.append(score)
    ...
    def max_tail_length(self,mol):
        nonlinkers = self.split_mol(mol)[1]
        if not nonlinkers:
            return 0

        for patt in self.bad_tail_patts:
            if any([m.HasSubstructMatch(patt) for m in nonlinkers]):
                return patt.GetNumAtoms() - 1
        return 0
