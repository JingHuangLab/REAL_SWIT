...
def parse_args():
    ...
    parser.add_argument("--smiles-type", "-s", help="Type of SMILES strings TYPES=(smiles,deepsmiles.*,scaffold) \
        [DEFAULT: smiles].", type=str, default="smiles")
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    # Make local modules available to Spark workers
    utils_zip = os.path.join(SCRIPT_DIR, "utils.zip")
    if os.path.exists(utils_zip):
        SC.addPyFile(utils_zip)
    else:
        raise FileNotFoundError(
            f"Cannot find {utils_zip}. Please run: cd {SCRIPT_DIR} && zip -r utils.zip utils"
        )
