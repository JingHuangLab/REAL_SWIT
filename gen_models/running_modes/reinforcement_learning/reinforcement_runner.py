...
class ReinforcementRunner:
    ...
    def run(self):
        ...
        #writer=open("/home/zhangky/tool/swit/examples/temp_loss_log/loss_log0116.csv",'w')  ### new added 240116
        #writer.write("step,loss,score,agent_ll,prior_ll,augment_ll\n")### new added 240116
        ...
        for step in range(self._config.n_steps):
            ...
            score = self._scaffold_filter.score(score_summary,step)
            ...
            loss = loss.mean()
            #current_loss=loss  ### new added 240116
            #current_loss=current_loss.cpu().detach().numpy() ### new added 240116
            ...
            #writer.write(f"{step},{current_loss},{np.mean(score)},{agent_likelihood.detach().mean()},{prior_likelihood.detach().mean()},{augmented_likelihood.detach().mean()}\n")
            memory_num = 0
            for i, scaffold in enumerate(self._scaffold_filter._scaffolds):
                for smi, score in self._scaffold_filter._scaffolds[scaffold].items():
                    memory_num+=1
            print("memory_num:{}".format(memory_num))
            if not self._config.n_mols==None:    
                if memory_num>self._config.n_mols:
                    break
        #writer.close()  ### new added 240116
