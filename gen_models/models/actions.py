...
class TrainModel(Action):
    ...
    def _epoch_iterator(self, dataloader):
        ...
        for padded_seqs in dataloader:
            padded_seqs = padded_seqs.to(device)
            loss = self.model.likelihood(padded_seqs).mean()
    ...
    def _initialize_dataloader(self, training_set):
        ...
        return tud.DataLoader(dataset, batch_size=self.batch_size, shuffle=True,
                              collate_fn=md.Dataset.collate_fn,generator=torch.Generator(device="cpu"))
...
class CollectStatsFromModel(Action):
    ...
    def run(self):
        ...
        # `model.sample_smiles` returns (list_of_smiles, array_of_likelihoods)
        sampled_smis, sampled_nlls = self.model.sample_smiles(num=self.sample_size)
        sampled_smis = np.array(sampled_smis)
...
class CalculateNLLsFromModel(Action):
    ...
    def run(self, smiles_list):
        ...
        for batch in dataloader:
            # move batch to model device before computing likelihood
