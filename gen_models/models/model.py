...
class RNN(tnn.Module):
    ...
    def __init__(self, vocabulary_size, num_dimensions=512, num_layers=3, cell_type='lstm', embedding_layer_size=256, dropout=0.,
        ...
        self._layer_size = num_dimensions
        ...
        self._embedding = tnn.Sequential(
            tnn.Embedding(self.vocabulary_size, self._embedding_layer_size),
            tnn.Dropout(self._dropout)
        )
        ...
        self._linear = tnn.Linear(self._layer_size, vocabulary_size)
    ...
    def forward(self, input_vector, hidden_state=None):  # pylint: disable=W0221
        ...
        if hidden_state is None:
            ...
            if self._cell_type == "gru":
                hidden_state = torch.zeros(*size, device=input_vector.device)
            else:
                hidden_state = [torch.zeros(*size, device=input_vector.device), torch.zeros(*size, device=input_vector.device)]
    ...
    def get_params(self):
        ...
        return {
            ...
            'num_dimensions': self._layer_size,
...
class Model:
    ...
    def load_from_file(cls, file_path: str, sampling_mode=False):
        ...
        if "layer_size" in network_params.keys():
            network_params["num_dimensions"]=network_params["layer_size"]
            network_params["vocabulary_size"]=34
            del network_params["layer_size"]
        model = Model(
    ...
    def _sample(self, batch_size=128) -> Tuple[torch.Tensor, torch.Tensor]:
        # ensure tensors are created on the same device as the network
